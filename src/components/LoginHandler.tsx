...
// Login logic has been moved to App.tsx handleLogin function.
    ...
    const login = async () => {
      try {
        ...
        if (!user || !user.username) {
          console.warn(
            "No Telegram username available on WebApp.initDataUnsafe.user"
          );
          return;
        }
        if (
          ...
          console.error(
            "Blocked insecure API call: page is HTTPS but API_BASE_URL uses HTTP. Update VITE_API_BASE_URL to https."
          );
          WebApp.showAlert(
            "Configuration error: insecure API endpoint blocked. Please use an HTTPS API URL."
          );
          return;
        }
        let country = "";
        ...
        if (!API_BASE_URL) {
          console.error(
            "VITE_API_BASE_URL is not defined. Set it in .env and rebuild."
          );
          ...
          if (data.message && data.message.includes("Welcome")) {
            try {
              ...
              if (
                err &&
                err.message &&
                err.message.includes("Popup is already opened")
              ) {
                console.info(
                  "Popup was already opened, skipping second popup."
                );
              } else {
        ...
        if (err && err.message && err.message.includes("Network Error")) {
          WebApp.showAlert(
            "Network error: check your API server and CORS configuration."
          );
        } else if (err && err.response && err.response.status === 0) {
          WebApp.showAlert(
            "Request blocked (mixed-content or CORS). See console for details."
          );
        } else {
          try {
